...
- **Account sign-in (Suite SSO, optional):** a `👤 Sign in` header button (username/password, optional TOTP 2FA), backed by `src/auth.js` + `src/db.js` + `src/totp.js` — the same accounts/sessions pattern already running in CryptoPro Charts and CryptoPro Suite. Point `DBCRYPTOCHARTS_POSTGRES_URL[_NON_POOLING]` at Charts' Supabase Postgres project (see `.env.example`) to share one login suite-wide. Without it, sign-in/register return 503 and the course works unaffected — progress always stays in browser `localStorage`. The "Enable 2FA" dialog shows a scannable QR code alongside the manual-entry secret.
